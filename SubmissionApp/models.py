...
class Submissions(models.Model):
    ...
    abstract    = models.TextField()  # <- résumé long
    ...
    status = models.CharField(max_length=20, choices=STATUS, default="submitted")  # <- DEFAULT
    ...
    def save(self, *args, **kwargs):
        ...
        # IMPORTANT : self.user doit être déjà défini AVANT full_clean()
        self.full_clean()
        super().save(*args, **kwargs)

    def clean(self):
        if not self.conference:
            raise ValidationError("La conférence est requise.")
